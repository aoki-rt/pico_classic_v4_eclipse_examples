  ...
  for (int i = 0; i < MAZESIZE_X; i++) {
    for (int j = 0; j < MAZESIZE_Y; j++) {
      wall[i][j].north = wall[i][j].east = wall[i][j].south = wall[i][j].west =
        _UNKNOWN;  //unknown information on the walls of the maze
    }
  }

  for (int i = 0; i < MAZESIZE_X; i++) {
    wall[i][0].south = WALL;               //set the south wall known at x=0,y=*
    wall[i][MAZESIZE_Y - 1].north = WALL;  //set the north wall known at x=0,y=15
  }

  for (int i = 0; i < MAZESIZE_Y; i++) {
    wall[0][i].west = WALL;               //set the west wall known at x=*,y=0
    wall[MAZESIZE_X - 1][i].east = WALL;  //set the east wall known at x=*,y=15
  }
  //   x,y
...
void MapManager::wallSet(bool IS_SEN_FR, bool IS_SEN_R, bool IS_SEN_L)  //record of wall information
{
  switch (mypos.dir) {
    case north:
